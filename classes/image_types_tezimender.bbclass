...
TEZI_DATA_SUFFIX ??= "datafs.tar.xz"
...
TEZI_EMMC_PARTITIONS ??= "bootfs rootfs"
TEZI_RAWNAND_UBIVOLUMES ??= "bootfs rootfs data"
...
def rootfs_tezi_emmc(d, partitions):
    from collections import OrderedDict
    offset_bootrom = d.getVar('OFFSET_BOOTROM_PAYLOAD') or ""
    offset_spl = d.getVar('OFFSET_SPL_PAYLOAD') or ""
...
def rootfs_tezi_rawnand(d, volumenames):
    from collections import OrderedDict
    imagename = d.getVar('IMAGE_LINK_NAME')
    ubivolumes = []
    ...
    if "data" in volumenames:
        ubivolumes.append(
              {
                "name": "data",
                "content": {
                  "filesystem_type": "ubifs",
                  "filename": imagename + "." + d.getVar('TEZI_DATA_SUFFIX'),
                  "uncompressed_size": get_uncompressed_size(d, "datafs")
                }
              })
...
python rootfs_tezi_run_json() {
    ...
    bootfs = "%s/%s.%s" % (d.getVar('IMGDEPLOYDIR'), d.getVar('IMAGE_LINK_NAME'), d.getVar('TEZI_BOOT_SUFFIX'))
    rootfs = "%s/%s.%s" % (d.getVar('IMGDEPLOYDIR'), d.getVar('IMAGE_LINK_NAME'), d.getVar('TEZI_ROOT_SUFFIX'))
    datafs = "%s/%s.%s" % (d.getVar('IMGDEPLOYDIR'), d.getVar('IMAGE_LINK_NAME'), d.getVar('TEZI_DATA_SUFFIX'))
    ...
    if flash_type == "rawnand":
        volumes = (d.getVar("TEZI_RAWNAND_UBIVOLUMES", d) or "").split()
        flash_data = rootfs_tezi_rawnand(d, volumes)

        uenv_file = d.getVar('UBOOT_ENV_TEZI_RAWNAND')
        artifacts += " " + uenv_file
        artifacts += " " + d.getVar('UBOOT_BINARY_TEZI_RAWNAND')
        if "bootfs" in volumes:
            artifacts += " " + bootfs
        if "rootfs" in volumes:
            artifacts += " " + rootfs
        if "data" in volumes:
            artifacts += " " + datafs
    elif flash_type == "emmc":
        partitions = (d.getVar("TEZI_EMMC_PARTITIONS", d) or "").split()
        flash_data = rootfs_tezi_emmc(d, partitions)

        uenv_file = d.getVar('UBOOT_ENV_TEZI_EMMC')
        artifacts += " " + uenv_file
        artifacts += " " + d.getVar('UBOOT_BINARY_TEZI_EMMC')

        # TODO: Multi image/raw NAND with SPL currently not supported
        if d.getVar('OFFSET_SPL_PAYLOAD'):
            artifacts += " " + d.getVar('SPL_BINARY')
        if "bootfs" in partitions:
            artifacts += " " + bootfs
        if "rootfs" in partitions:
            artifacts += " " + rootfs
...
TAR_IMAGE_ROOTFS_task-image-datafs = "${WORKDIR}/datafs"
IMAGE_CMD_datafs () {
	cp -a ${IMAGE_ROOTFS}/data/* ${WORKDIR}/datafs
}
do_image_datafs[dirs] += "${WORKDIR}/datafs"
do_image_datafs[cleandirs] += "${WORKDIR}/datafs"

TEZI_IMAGE_TEZIIMG_PREFUNCS ??= "rootfs_tezi_run_json"
IMAGE_TYPEDEP_tezimender += "${TEZI_BOOT_SUFFIX} ${TEZI_ROOT_SUFFIX} ${TEZI_DATA_SUFFIX}"
IMAGE_CMD_tezimender () {
	...
	# second transform "moves" them in a subfolder ${TEZI_IMAGE_NAME}-Tezi_${PV}-${DATE}.
	${IMAGE_CMD_TAR} \
		--transform='s/.*\///' \
		--transform 's,^,${TEZI_IMAGE_NAME}-Tezi_${PV}-${DATE}/,' \
		-chf ${IMGDEPLOYDIR}/${TEZI_IMAGE_NAME}-Tezi_${PV}-${DATE}.tar \
		toradexlinux.png marketing.tar prepare.sh wrapup.sh \
		${WORKDIR}/image-json/image.json ${TEZI_ARTIFACTS}
}
do_image_tezimender[dirs] += "${WORKDIR}/image-json ${DEPLOY_DIR_IMAGE}"
do_image_tezimender[cleandirs] += "${WORKDIR}/image-json"
do_image_tezimender[prefuncs] += "${TEZI_IMAGE_TEZIIMG_PREFUNCS}"
do_image_tezimender[recrdeptask] += "do_deploy"
do_image_tezimender[vardepsexclude] = "DATE"
